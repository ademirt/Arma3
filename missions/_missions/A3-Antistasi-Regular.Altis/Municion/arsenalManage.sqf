...
if ({_x == _lockedWeapon} count _armasTrad >= (_defWeap + (count unlockedWeapons) - (2*_fabricas))) then
	{
	_desbloquear = false;
...
if (not(_item in unlockedItems)) then {
	_cuenta = -63;
	if (_item in genItems) then {_cuenta = -53};
	if (hayACE) then {_cuenta = _cuenta - 31};
	if ((_x in genVests) && ({_x == _item} count _items >= (_cuenta - 10 + (count unlockedItems) - _fabricas))) then {
		if (hayBE) then {["unl_wpn"] remoteExec ["fnc_BE_XP", 2]};
