...

{
	...
	["skill0",[	
		...
		["aimingShake",0.05,0.10],
		["aimingSpeed",0.05,0.10],
		["spotDistance",0.05,0.10],
		["spotTime",0.05,0.10],
		["courage",0.30,0.50],
		["reloadSpeed",0.05,0.10],
		["commanding",0.05,0.10],
		["general",0.05,0.10]
	]],
	["skill1",[	
		["aimingAccuracy",0.05,0.10],
...
if (A3XAI_verifySettings) then {
	...
	if !(A3XAI_side in [east,west]) then {diag_log format ["[A3XAI] Error found in variable A3XAI_side. User defined value: %1. Acceptable values: east, west",A3XAI_side]; A3XAI_side = west;};
};
