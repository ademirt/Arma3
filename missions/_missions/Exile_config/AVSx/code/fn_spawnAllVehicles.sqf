...
if (AVS_useSpawnedPersistentVehiclesRandom) then
{
	...
	_cont = 0; 
	{
		...
		while {(_spawnedVehicleTracker select 1) < _numToSpawn} do
		{
		    ...
		    //_dbm = createMarker ["marker_"+str _cont, _positionReal];
			//_dbm setMarkerColor "ColorBlue";
			//_dbm setMarkerType "mil_objective";
			
			//diag_log format["AVS incrementando %1",_cont];
			//_cont = _cont + 1; 
