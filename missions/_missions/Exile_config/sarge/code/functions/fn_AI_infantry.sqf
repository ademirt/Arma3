...
// diag_log format ["SARGE: %1 , %2, %3 ",_grouptype,_ai_type,_side];
...
_wmin = 20;
_wmax = 2;
_wblack = [[[2092,14167],[10558,12505]]];
_rndpos = [_wcenter, 0, _wradious, _wmin, 0, _wmax, 0, _wblack] call BIS_fnc_findSafePos;
...
_leader = _group createunit [_leaderList call BIS_fnc_selectRandom, [(_rndpos select 0) , _rndpos select 1, 0], [], 0.5, "CAN_COLLIDE"];
 ...
 //diag_log format ["SARGE OK!!!!!: %1 , %2, %3 ",_grouptype,_ai_type,_side];
...
//teste ademir
_marker1 = createMarker [format["mark_0%1",time], getpos _leader];
_marker1 setMarkerType "hd_objective";
[_leader,_marker1] spawn {
  _a = _this select 0;
  _mrk = _this select 1;
