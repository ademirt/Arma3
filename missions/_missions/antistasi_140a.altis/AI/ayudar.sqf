...
_isPlayer =
  if ({
    ...
    count units group _unit > 0) then {
    true
  } else {
    false
  };
...
if (_medico != _unit) then {
  ...
  if (hasInterface) then {
    if (player == _unit) then {
      hint format["%1 is on the way to help you", name _medico]
    }
  };
  ...
  if ((isPlayer _unit) and !(isMultiplayer)) then {
    if (([_medico] call A3A_fnc_canFight) and(_medico distance _unit > 3) and(_medico == _unit getVariable["ayudado", objNull]) and!(_unit getVariable["llevado", false]) and(allUnits findIf {
        ((side _x == malos) or(side _x == muyMalos)) and(_x distance2D _unit < 50)
      } == -1)) then {
      _medico setPos position _unit
    };
  };
  if ((_unit distance _medico <= 3) and(alive _unit) and([_medico] call A3A_fnc_canFight) and(_medico == vehicle _medico) and(_medico == _unit getVariable["ayudado", objNull]) and(isNull attachedTo _unit) and!(_medico getVariable["cancelRevive", false])) then {
    if ((_unit getVariable["INCAPACITATED", false]) and(!isNull _enemy) and(_timeOut >= time) and(_medico != _unit)) then {
      _cobertura = [_unit, _enemy] call A3A_fnc_cobertura; {
        if (([_x] call A3A_fnc_canFight) and(_x distance _medico < 50) and!(_x getVariable["ayudando", false]) and(!isPlayer _x)) then {
          [_x, _enemy] call A3A_fnc_fuegoSupresor
        }
      }
      forEach units(group _medico);
      if (count _cobertura == 3) then {
        ...
        waitUntil {
          sleep 0.3;
          ((animationState _medico) == "AmovPercMstpSlowWrflDnon_AcinPknlMwlkSlowWrflDb_2") or((animationState _medico) == "AmovPercMstpSnonWnonDnon_AcinPknlMwlkSnonWnonDb_2") or!([_medico] call A3A_fnc_canFight) or(_timeOut < time)
        };
        if ([_medico] call A3A_fnc_canFight) then {
          ...
          [_medico] spawn {
            sleep 4.5;
            (_this select 0) playMove "AcinPknlMwlkSrasWrflDb"
          };
          _timeOut = time + 30;
          while {
            true
        ...
        if ((alive _unit) and([_medico] call A3A_fnc_canFight) and(_medico == vehicle _medico) and!(_medico getVariable["cancelRevive", false])) then {
          ...
          if (_curado) then {
            if (_medico != _unit) then {
              if (_isPlayer) then {
                _medico groupChat format["You are ready %1", name _unit]
              }
            };
          };
          sleep 5;
          ...
          if ((alive _unit) and(_unit getVariable["INCAPACITATED", false])) then {
            ...
            waitUntil {
              sleep 0.3;
              (lifeState _unit != "INCAPACITATED") or(_timeOut < time)
            };
      ...
      if ((animationState _medico == "amovpknlmstpsraswrfldnon") or(animationState _medico == "AmovPercMstpSlowWrflDnon_AcinPknlMwlkSlowWrflDb_2") or(animationState _medico == "AmovPercMstpSnonWnonDnon_AcinPknlMwlkSnonWnonDb_2")) then {
        _medico switchMove ""
      };
    }
    else {
  ...
  if (_medico == _unit getVariable["ayudado", objNull]) then {
