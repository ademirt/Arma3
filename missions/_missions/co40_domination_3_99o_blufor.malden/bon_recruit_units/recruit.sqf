...
if (_unittype == "B_Fighter_Pilot_F") then {	
	...
	private _jat = ["B_Plane_CAS_01_F","B_Plane_Fighter_01_F","	B_Plane_Fighter_01_Stealth_F"];
...
if (_unittype == "B_helipilot_F") then {
	...
	private _helitransport = ["B_Heli_Transport_01_F","B_Heli_Transport_01_camo_F"];
...
if (_unittype == "B_Captain_Jay_F") then {
	...
	//create group and aircraft
	private _heli_at = createVehicle ["B_Heli_Attack_01_F", [0,0,0], [], 500, "FLY"];
...
//if (worldName == "Tanoa") then { _unit forceAddUniform "U_I_FullGhillie_lsh"; } else { _unit forceAddUniform "U_B_FullGhillie_ard";};	
