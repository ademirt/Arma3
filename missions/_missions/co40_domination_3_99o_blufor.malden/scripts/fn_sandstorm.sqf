...
while {true} do {
	if (rain isEqualTo 0 && {!d_rscspect_on && {!d_still_in_intro && {!surfaceIsWater (getPosWorld player) && {((getPos player) # 2) < 15 && {!(player getVariable "xr_pluncon") && {!(player getVariable ["ace_isunconscious", false])}}}}}}) then {
