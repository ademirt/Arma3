...
d_x_sm_pos = "d_sm_143" call d_fnc_smmapos; //  destroy lost uav
	...
	while {true} do {
		...
		if (_time_over > 0) then {
			...
			if (!_enemy_created) then {
				_trigger = [_exact_pos, [20,20,0,false], [d_enemy_side,"PRESENT",false], ["this","d_cruise_m_enemy = true",""]] call d_fnc_CreateTrigger;
				_enemy_created = true;
				_estart_pos = [_exact_pos,300] call d_fnc_GetRanPointCircleOuter;
				_unit_array = ["basic", d_enemy_side] call d_fnc_getunitlistm;
