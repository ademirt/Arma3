...
d_x_sm_pos = "d_sm_46" call d_fnc_smmapos; // index: 45,   Destroy factory building in Lalezar
d_x_sm_type = "normal"; // "convoy"

if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_864a";
	d_current_mission_resolved_text = localize "STR_DOM_MISSIONSTRING_865";
};

if (call d_fnc_checkSHC) then {
	private _poss = d_x_sm_pos select 0;
	private _objs = _poss nearObjects ["Land_Ind_TankBig", 50]; // d_sm_land_tankbig
	sleep 2.123;
