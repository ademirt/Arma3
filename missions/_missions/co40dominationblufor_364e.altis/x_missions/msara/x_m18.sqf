...
if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_1597";
	d_current_mission_resolved_text = localize "STR_DOM_MISSIONSTRING_746";
};

if (call d_fnc_checkSHC) then {
	private _poss = d_x_sm_pos select 0;
	["aa", 1, "tracked_apc", 1, "tank", 1, d_x_sm_pos select 1, 1, 120, true] spawn d_fnc_CreateArmor;
	sleep 2.123;
	["specops", 2, "allmen", 1, _poss, 100, true] spawn d_fnc_CreateInf;
	sleep 2.111;
