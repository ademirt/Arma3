...
if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_1606";
	d_current_mission_resolved_text = localize "STR_DOM_MISSIONSTRING_729";
};

if (call d_fnc_checkSHC) then {
	d_x_sm_pos _poss = x_sm_pos select ((floor random 14) + 1);
	...
	sleep 2.123;
	["aa", 1, "tracked_apc", 1, "tank", 0, d_x_sm_pos select 15, 1, 130, true] spawn d_fnc_CreateArmor;
	sleep 2.123;
	["specops", 1, "allmen", 0, d_x_sm_pos select 15, 80, true] spawn d_fnc_CreateInf;
	sleep 2.123;
	["aa", 1, "tracked_apc", 1, "tank", 0, d_x_sm_pos select 16, 1, 130, true] spawn d_fnc_CreateArmor;
	sleep 2.123;
	["specops", 1, "allmen", 0, d_x_sm_pos select 16, 80, true] spawn d_fnc_CreateInf;
	sleep 2.123;
	["aa", 1, "tracked_apc", 1, "tank", 0, d_x_sm_pos select 17, 1, 130, true] spawn d_fnc_CreateArmor;
	sleep 2.123;
	["specops", 1, "allmen", 0, d_x_sm_pos select 17, 80, true] spawn d_fnc_CreateInf;
	sleep 2.123;
	["aa", 1, "tracked_apc", 1, "tank", 0, d_x_sm_pos select 18, 1, 130, true] spawn d_fnc_CreateArmor;
	sleep 2.123;
	["specops", 1, "allmen", 0, d_x_sm_pos select 18, 80, true] spawn d_fnc_CreateInf;
	sleep 2.123;
};
