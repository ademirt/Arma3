...
if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_1607";
...
if (call d_fnc_checkSHC) then {
	...
	sleep 2.012;
	["aa", 1, "tracked_apc", 0, "tank", 0, d_x_sm_pos select 1, 1, 0] spawn d_fnc_CreateArmor;
	sleep 2.012;
	["aa", 0, "tracked_apc", 1, "tank", 0, d_x_sm_pos select 1, 1, 0] spawn d_fnc_CreateArmor;
	sleep 2.123;
	["specops", 1, "allmen", 1, d_x_sm_pos select 3, 60] spawn d_fnc_CreateInf;
};
