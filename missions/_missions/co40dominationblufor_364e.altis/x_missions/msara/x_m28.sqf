...
if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_1609";
...
if (call d_fnc_checkSHC) then {
	...
	sleep 2.333;
	["specops", 1, "allmen", 2, d_x_sm_pos select 2, 80, true] spawn d_fnc_CreateInf;
	sleep 2.333;
	["aa", 0, "tracked_apc", 1, "tank", 1, d_x_sm_pos select 3, 1, 100, true] spawn d_fnc_CreateArmor;
};
