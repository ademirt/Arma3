...
if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_1611";
...
if (call d_fnc_checkSHC) then {
	...
	_vec setPos _poss;
	sleep 2.123;
	["specops", 1, "allmen", 1, d_x_sm_pos select 1, 100, true] spawn d_fnc_CreateInf;
	sleep 2.321;
	["aa", 1, "tracked_apc", 1, "tank", 0, d_x_sm_pos select 2, 1, 200, true] spawn d_fnc_CreateArmor;
