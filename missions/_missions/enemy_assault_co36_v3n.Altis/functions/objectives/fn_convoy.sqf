...
_convoygrp = creategroup _side;
	...
	for "_w" from 1 to (3) do {
		...
		if (_markwps) then {
			...
			_marker = [_wppos,"COLORRED",_marktext] call fnc_ghst_mark_point;
		};
	};
	...
	for "_e" from 1 to (_enum) do {
		...
		if (_markwps) then { 
			_marker = [_wppos,"COLORRED","START"] call fnc_ghst_mark_point; 
		};
		...
		if (_emptyseats > 12) then {_emptyseats = 12;};
			...
			for "_s" from 1 to _emptyseats do {
				...
				_man = [_wppos,_cargogrp,_mansel,aiSkill] call fnc_ghst_create_unit;
	...
	};
	{
		[_x] spawn {
			_x = _this select 0;
			_marker = [_x,"COLORRED"] call fnc_ghst_mark_point;
...
_trig1act = format ["['%1','succeeded'] call BIS_fnc_taskSetState; [playableunits,5000,100] remoteExec ['fnc_ghst_addscore'];", _tsk];
...
[_position_mark,500] spawn fnc_ghst_full_cleanup;