...
while { GRLIB_endgame == 0 } do {
	...
	if ( count _usable_sectors > 0 ) then {
		...
		if ( random 100 < 33) then {
			...
			_civveh addEventHandler ["HandleDamage", { private [ "_damage" ]; 
			if (( side (_this select 3) != GRLIB_side_friendly ) && ( side (_this select 3) != GRLIB_side_enemy )) then { _damage = 0 } else { _damage = _this select 2 }; _damage } ];
