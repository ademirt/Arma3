...
while { GRLIB_endgame == 0 } do {
	...
	if (_is_infantry) then {
		...
		{
			_x createUnit [_sector_spawn_pos, _grp,"this addMPEventHandler [""MPKilled"", {_this spawn kill_manager}]; this addEventHandler [""HandleDamage"",{_damage = (_this select 2)*1.85; _damage}];", 0.5, "private"];//teste
		} foreach _squad;
	} else {
