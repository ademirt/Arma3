...
_grp = createGroup GRLIB_side_enemy;
{
	if ( ( count units _grp ) < _corrected_amount) then {
		_x createUnit [_spawnpos, _grp,'this addMPEventHandler ["MPKilled", {_this spawn kill_manager}]; this addEventHandler ["HandleDamage",{_damage = (_this select 2)*1.85; _damage}]'];
