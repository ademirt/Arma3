...
_air2_array = [[(_spawnmark select 0) + (cos _dir * 100), (_spawnmark select 1) - (sin _dir * 50), _flyheight + 100], _dir, _airtypesel, _escortgrp] call BIS_fnc_spawnVehicle;
...
_air2 flyInHeight _flyheight + 100;

//escort helicopter 2
_airtypesel2 = _escortarray call BIS_fnc_selectRandom;  
_air3_array = [[(_spawnmark select 0) + (cos _dir * 50), (_spawnmark select 1) - (sin _dir * 100), _flyheight + 100], _dir, _airtypesel2, _escortgrp] call BIS_fnc_spawnVehicle;
...
_air3 flyInHeight _flyheight + 100;
...
_smoke = "SmokeShellGreen" createVehicle _pos;
	...
	if ((alive _air1) and (canmove _air1) and (alive (driver _air1))) then {
		...
		ghst_rtb_transport = [_air1,["<t size='1.5' shadow='2' color='#00FFFF'>Helicopter RTB</t> <img size='3' color='#00ff00'", "call ghst_fnc_rtb_transport", [_air1,_flyheight,_spawnmark,_escortgrp,[_air1,_air2,_air3]], 5, false, true, "","alive _target"]] remoteExec ["addAction"];
	};
