...
0 spawn {
	...
	addMissionEventHandler ["Draw3D", {
		...
		if (_pos_cam distance2D d_pl_flag_base < 1000) then {
		...
		};
		{
			if (alive _x && {isTouchingGround _x}) then {
				...
				if (_distp < 150) then {
					...
					__scale3;
					drawIcon3D ["#(argb,8,8,3)color(0,0,0,0)", [0,0,1,_alpha], _pos, 1, 1, 0, format ["MHQ %1", _x getVariable "d_ma_text"], 1, _scale, "RobotoCondensed"];
				};
			};
			false
		} count d_mhq_3ddraw;
