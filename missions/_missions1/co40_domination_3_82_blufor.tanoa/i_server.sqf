...
d_bonus_air_positions_carrier = [];
{
	...
	deleteMarker _x;
	false
} count (allMapMarkers select {_x select [0, 22] == "d_bonus_air_positions_"});
{
	d_bonus_air_positions_carrier pushBack [markerPos _x, markerDir _x];
	deleteMarker _x;
	false
} count (allMapMarkers select {_x select [0, 30] == "d_bonus_air_positions_carrier_"});
...
d_bonus_vec_positions = [];
{
	...
	deleteMarker _x;
	false
} count (allMapMarkers select {_x select [0, 22] == "d_bonus_vec_positions_"});
#else
...
d_bonus_air_positions_w = [];
{
	...
	deleteMarker _x;
	false
} count (allMapMarkers select {_x select [0, 24] == "d_bonus_air_positions_w_"});
...
d_bonus_air_positions_e = [];
{
	...
	deleteMarker _x;
	false
} count (allMapMarkers select {_x select [0, 24] == "d_bonus_air_positions_e_"});
...
d_bonus_vec_positions_w = [];
{
	...
	deleteMarker _x;
	false
} count (allMapMarkers select {_x select [0, 24] == "d_bonus_vec_positions_w_"});
...
d_bonus_vec_positions_w = [];
{
	...
	deleteMarker _x;
	false
} count (allMapMarkers select {_x select [0, 24] == "d_bonus_vec_positions_e_"});
#endif
...
if (d_with_ai) then {
	...
	if (!d_carrier) then {
		...
		d_AI_HUT setPosASL [(d_pos_ai_hut select 0) select 0, (d_pos_ai_hut select 0) select 1, d_carrier_height];
	};
