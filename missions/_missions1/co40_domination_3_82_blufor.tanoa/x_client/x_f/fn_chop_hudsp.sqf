				...
				if (d_chophud_on && {!visibleMap}) then {
...
#ifndef __TT__
									(!isNull _liftobj && {_isvalid  && {(getPosVisual _vec) select 2 > 2.5}})
#else
									(!isNull _liftobj && {_isvalid && {(getPosVisual _vec) select 2 > 2.5 && {_liftobj getVariable ["d_side", d_player_side] == d_player_side}}})
#endif
								};
							};
						};
					...
					if (_check_cond) then {
						...
						if !(_vec getVariable ["d_vec_attached", false]) then {
							...
							//private _liftobj_pos = getPosVisual _liftobj;
							//private _pos_vec = getPosVisual _vec;
							...
							if (_vec inArea [_liftobj, 10, 10, 0, false] && {getPosVisual _vec select 2 < _lift_height}) then {
								__CTRL2(64448) ctrlSetText __ui_ok;
							} else {
								__CTRL2(64442) ctrlSetText (if (getPosVisual _vec select 2 >= _lift_height) then {localize "STR_DOM_MISSIONSTRING_198"} else {""});
							...
							private _ddx1 = [round ((getPosASLVisual _liftobj) select 2), round ((getPosVisual _liftobj) select 2)] select (!surfaceIsWater (getPosASL _liftobj));
