		...
		if (!(_chopper getVariable ["d_vec_attached", false]) && {_pos select 2 > 2.5 && {_pos select 2 < 11}}) then {
...
#ifndef __TT__
					if (!_isvalid || {speed _liftobj > 10 || {(getPosVisual _liftobj) select 2 > 5}}) then {_liftobj = objNull};
#else
					if (!_isvalid || {speed _liftobj > 10 || {(getPosVisual _liftobj) select 2 > 5 || {_liftobj getVariable ["d_side", d_player_side] != d_player_side}}}) then {_liftobj = objNull};
#endif
				};
			};
			sleep 0.1;
			if (!isNull _liftobj && {_liftobj != _chopper getVariable ["d_Attached_Vec", false]}) then {
				if !(_liftobj getVariable ["d_MHQ_Deployed", false]) then {
					//_liftobj_pos = getPosVisual _liftobj;
			...
			if (isNull _liftobj) then {
				...
				if (_chopper getVariable ["d_vec_attached", false]) then {
					...
					[_liftobj, false] remoteExecCall ["engineOn", _liftobj];
					if ((getPosVisual _liftobj) select 2 > 5) then {
						_liftobj spawn {
							while {(getPosVisual _this) select 2 > 5} do {
					...
					if (!(_liftobj isKindOf "StaticWeapon") && {(getPosVisual _liftobj) select 2 < 200}) then {
						waitUntil {sleep 0.222;(getPosVisual _liftobj) select 2 < 10};
					} else {
