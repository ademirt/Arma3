...
if ({alive _x} count units (group player) > 0) then {
	if (isNil "_this" || {_this isEqualTo []}) then {
		...
		private _hasunits = false;
		{
			...
			_x setPosASL [_np_unit select 0, _np_unit select 1, _posasl_p select 2];
			sleep 0.2;
			_x reveal player;
			_x doFollow player;
			_hasunits = true;
			false
		} count ((formationMembers player) select {!isPlayer _x && {alive _x && {isNull objectParent _x && {_x distance2D _posasl_p > 500}}}});
		if (_hasunits) then {
			private _wps = waypoints (group player);
			if !(_wps isEqualTo []) then {
				(_wps select 0) setWPPos (getPos player);
				_wps spawn {
					sleep 0.3;
					{
						deleteWaypoint _x;
		...
		private _dir = getDir player;
		{
			...
			[_x] spawn {
				...
				if (alive _unit && {(getPosATL _unit) select 2 > 1}) then {[_unit, 0] call d_fnc_SetHeight};
			};
			false
