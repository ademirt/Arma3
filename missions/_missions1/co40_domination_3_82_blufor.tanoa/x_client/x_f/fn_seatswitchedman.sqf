...
if (_not_allowed) then {
	...
	if (!isNull _unit2 && {!(_old_assigned isEqualTo [])}) then {
		...
		if (_co == 1) then {
			call {
				if (_old_assigned select 0 == "Driver") exitWith {_unit2 moveInDriver _vec};
				if (_old_assigned select 0 == "Cargo") exitWith {_unit2 moveInCargo _vec};
			};
		} else {
			if (_co > 1) then {
				_unit2 moveInTurret [_vec, _old_assigned select 1];
			};
		};
	};
