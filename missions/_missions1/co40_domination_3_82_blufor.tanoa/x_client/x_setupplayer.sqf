...
d_name_pl = player call d_fnc_getplayername;
...
diag_log ["Internal D Version: 3.82"];
...

{
	...
	if (!d_carrier) then {
		...
		if (_x select 0 distance2D D_FLAG_BASE < 100) then {
			private _temppos = _x select 0;
			_temppos set [2, getPosASL D_FLAG_BASE select 2];
			_box setPosASL (_x select 0);
		} else {
			_box setPos (_x select 0);
		};
	};	
...
#ifdef __IFA3LITE__
{
	private _ar = _x;
	{
		if (_x != "FirstAidKit" && {_x != "Medikit" && {_x != "ItemMap" && {_x != "ItemRadio" && {_x != "ItemCompass" && {_x != "ItemWatch"}}}}}) then {
			...
			if (_kind != "") then {
				private _al = configSourceAddonList (configFile >> _kind >> _x);
				if ((_al select 0) select [0, 4] != "WW2_") then {
