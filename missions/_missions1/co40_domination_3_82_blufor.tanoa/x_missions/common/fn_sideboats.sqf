...
for "_ii" from 0 to 3 do {
	private _boat = createVehicle [_boatt, _posi_array select _ii, [], 0, "NONE"];
	_boat setDir (_dirs select _ii);
	_boat setPos (_posi_array select _ii);
...
for "_i" from 4 to 7 do {
	["specops", 1, "allmen", 1, _posi_array select _i, 100, true] spawn d_fnc_CreateInf;
	sleep 2.333;
};
