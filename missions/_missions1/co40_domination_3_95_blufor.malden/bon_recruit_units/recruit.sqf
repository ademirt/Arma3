...
if (_unittype == "B_Fighter_Pilot_F") then {	
	...
	private _jat = ["B_Plane_CAS_01_F","I_Plane_Fighter_03_CAS_F","B_Plane_Fighter_01_F","B_Plane_Fighter_01_Stealth_F"];
...
if (_unittype == "B_Helipilot_F") then {
	...
	private _helitransport = ["B_Heli_Transport_03_F","B_Heli_Transport_01_F"];
	
	if (isClass(configFile >> "cfgPatches" >> "rhsusf_main")) then
	{
		_helitransport append ["RHS_UH1Y_d","RHS_CH_47F_10","RHS_UH60M"];
	};
	
	[West, "HQ"] sideChat "Transport on the way!!";
	
	private _ve_heli = _helitransport call BIS_fnc_selectRandom;
	//create group and aircraft
	private _heli_TR = createVehicle [_ve_heli, [0,0,0], [], 5000, "FLY"];
