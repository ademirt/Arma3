...
if (markerPos _mname isEqualTo [0,0,0]) then {
	[_mname, _this select 1, "ICON", "ColorBlue", [0.4,0.4], format [localize "STR_DOM_MISSIONSTRING_910", _unit call d_fnc_getplayername], 0, "mil_marker"] call d_fnc_CreateMarkerGlobal;
	if (d_tt_ver) then {
