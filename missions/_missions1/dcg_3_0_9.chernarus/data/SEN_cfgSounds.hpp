class SEN_transportFlyby {
	name = "";
	sound[] = {"media\transport_flyby.ogg", 3.5, 1};
	titles[] = {};
};
class SEN_drill {
	name = "";
	sound[] = {"media\drill.ogg", 3, 1};
	titles[] = {};
};
// credits - Bohemia Interactive
class SEN_transportAcknowledged {
	name = "";
	sound[] = {"media\transport_acknowledged.ogg", 2, 1};
	titles[] = {};
};
class SEN_transportWelcome {
	name = "";
	sound[] = {"media\transport_welcome.ogg", 4.5, 1};
	titles[] = {};
};
class SEN_transportAccomplished {
	name = "";
	sound[] = {"media\transport_accomplished.ogg", 4.5, 1};
	titles[] = {};
};
class SEN_transportDestroyed {
	name = "";
	sound[] = {"media\cas_heli_destroyed.ogg", 2, 1};
	titles[] = {};
};

// credits - Tier1Ops
class confirm1 {
	name = "";
	sound[] = {"media\confirm1.ogg", 1, 1};
	titles[] = {};
};
class warning1 {
	name = "";
	sound[] = {"media\warning1.ogg", 1, 1};
	titles[] = {};
};

// credits - cobra4v320
class button_click {
	name = "";
	sound[] = {"media\click.ogg", 1, 1};
	titles[] = {};
};
class wire_cut {
	name = "";
	sound[] = {"media\cutting.ogg", 1, 1};
	titles[] = {};
};