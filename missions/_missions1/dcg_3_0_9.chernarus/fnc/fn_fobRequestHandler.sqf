...
if (count _package isEqualTo 1) exitWith {
	[_package select 0] remoteExec ["SEN_fnc_fobReceiveRequest", owner (getAssignedCuratorUnit SEN_curatorFOB), false];
};
...
[_userName,_response] remoteExec ["SEN_fnc_fobAnswerRequest", owner _requestor, false];