...
call {
    if (_baseType isEqualTo 1) exitWith {
    	_base = [_pos] call SEN_fnc_spawnBaseSmall;
    	_return pushBack _base;
    };
    if (_baseType isEqualTo 2) exitWith {
    	_base = [_pos] call SEN_fnc_spawnBase;
    	_return pushBack _base;
    };
    _return pushBack [];
};
...
if (random 1 < _vehChance) then {
    _vehArray = [[_pos, 100, 180, 6, 0, 1, 0,[],[_pos,_pos]] call BIS_fnc_findSafePos,1,1,_side,_uncache] call SEN_fnc_spawnGroup;
