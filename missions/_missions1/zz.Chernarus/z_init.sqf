...
_zombie setdammage 0.99999;
_smokepos = 0;
_Zfirednear = 0;
_zweapon_player = "weapon";
...
_zombie addEventHandler["FiredNear", {_Zfirednear = 1; _zweapon_player = (_this select 3); hint format["arma:::: %1",_zweapon_player];}];
...
if (isNil "opx_fnc_findZombieTarget") then 
{
    ...
    opx_fnc_playerNoise = 
	{
        ...
        _rating = _rating + _surfaceValue;
   		...
   		if (_Zfirednear == 1) then
		{
            hint " ==TIROOOO "; sleep 0.95;
		    _Zfirednear = 0;
            ...
            if (_val1 < 90) then {
			   ...
			   _silenced = false;
				...
				if (_zweapon_player == primaryWeapon _player) then {
					...
					if (_zweapon_player == handgunWeapon _player) then {
						for "_i" from 0 to ((count _silencers) - 1) do {
							{
								if (_x == (_silencers select _i)) then {
									_silenced = true;
			...
			hint format [" ***tiro rating: %1",_rating]; sleep 1;
        };
    ...
    opx_fnc_findZombieTarget = {
	    //hint "opx_fnc_findZombieTarget ..";sleep 0.8;
...
while {zombieDoWork} do 
{
    ...
    while {zombieDoWork} do 
	{
        ...
        while {zombieDoWork} do 
		{
            ...
            if (!isNull _target && _target distance _zombie < 4) then {
                _target = _target;
            } else {
...
hint "  ACHOU TARGET 1";sleep 0.8;
                ...
                if ((vehicle _target iskindof "man") && _zombie distance _target < Ryanzombiesattackdistance) then {
					...
					hint "  ACHOU TARGET ABC";sleep 0.8;
					
                        breakTo "findtarget";
                    };
                };
hint "  ACHOU TARGET 2";sleep 0.8;				
...
hint "  ACHOU TARGET 3";sleep 0.8;				
				...
				hint format ["  SLEEP  X (%1)... FIND TARGET",_x];
                Sleep _x;
			...
			hint format [" PERDEU TARGET A=%1, B=%2 ",_a,_b];
            if (_a > _b) then {
                breakTo "loop"
            };
        };
